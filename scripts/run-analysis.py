...
'''
        ...
        prompt = (
            "执行 Path A 完整定时分析工作流（Step 1 锁获取已由脚本完成）：\n"
            "- Step 2: 读取 portfolio.csv 和 recommendations.csv 到上下文\n"
            "- Step 2.5 (快照校准): 执行 `python3 scripts/snapshot.py calibrate`，"
            "对比当前持仓与上期快照，若 quantity 下降则自动将 pending 的 reduce/sell 建议"
            "标记为 verified (outcome=correct(user_acted))。"
            "首次运行（无快照）静默跳过。记录校准结果供 Step 5 报告使用。\n"
            ...
            "- Step 5 (含冷却期过滤): 执行 `python3 scripts/snapshot.py cooldown` "
            "获取冷却期标的清单。冷却期内（30天内已有correct(user_acted)记录）的标的，"
            "不生成同方向的 reduce/sell 建议（除非价格相对成本已跌 ≥20%）。"
            "生成完整报告写入 reports/analysis-YYYY-MM-DD-HHmm.md"
            "（含市场概况、历史回顾、建议执行确认表、持仓诊断、建议表、风险提示）\n"
            ...
            "若无触发标的，跳过此步。\n"
            "- Step 7b (飞书文档同步):\n"
            "  a. 检查环境变量 $FEISHU_REPORT_FOLDER 是否已设置，"
            "若未设置则跳过此步并记录日志\n"
            "  b. 执行以下命令创建飞书文档（用 python 读取报告文件内容，再传给 lark-cli，"
            "避免 shell 转义和 @file 语法问题）。"
            "将 YYYY-MM-DD HHmm 和 YYYY-MM-DD-HHmm 替换为当前报告的实际时间戳：\n\n"
            "  python3 -c \"\nimport subprocess, os\n"
            "report_path = 'reports/analysis-YYY-MM-DD-HHmm.md'\n"
            "content = open(report_path).read()\n"
            "subprocess.run([\n"
            "    'lark-cli', '--profile', 'finance-agent', 'docs', '+create',\n"
            "    '--title', '投资分析报告 YYYY-MM-DD HHmm',\n"
            "    '--folder-token', os.environ['FEISHU_REPORT_FOLDER'],\n"
            "    '--markdown', content,\n"
            "], check=True)\n\"\n\n"
            "  c. 从响应中提取 doc_url，供下一步使用\n"
            "- Step 7c (报告推送): 通过 lark-cli --profile finance-agent --as bot im +messages-send "
            "--user-id \"$ALLOWED_OPEN_ID\" --msg \"...\" 推送报告摘要到飞书，"
            "**消息结尾必须附上 Step 7b 中创建的报告文档链接**，"
            "格式: \"\\n\\n📄 完整报告: <doc_url>\"\n"
            "- Step 7d (日志): 日志写入 agent.log\n"
            "- Step 7e (保存快照): 执行 `python3 scripts/snapshot.py save`，"
            "保存本轮 portfolio.csv 快照到 snapshots/ 目录，供下一轮校准使用。"
            "自动清理超过 60 份的旧快照。\n\n"
            "用中文生成所有输出和报告。\n\n"
