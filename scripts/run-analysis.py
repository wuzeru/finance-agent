...
'''
        ...
        prompt = (
            ...
            "若未设置则跳过此步并记录日志\n"
            "  b. 用 lark-cli --profile finance-agent docs +create "
            "--title \"投资分析报告 YYYY-MM-DD HHmm\" "
            "--folder-token \"$FEISHU_REPORT_FOLDER\" "
            "--markdown \"$(cat reports/analysis-YYYY-MM-DD-HHmm.md)\" 创建飞书文档\n"
            "  c. 若内容过长导致创建失败，改用两步方案:\n"
            "    - 先用 docs +create --title \"...\" --folder-token \"$FEISHU_REPORT_FOLDER\" "
            "--markdown \"# 投资分析报告 YYYY-MM-DD HHmm\" 创建文档框架\n"
            "    - 再用 docs +update --doc \"<doc_id>\" --mode append "
            "--markdown \"$(cat reports/xxx.md | tail -n +2)\" 追加全部内容\n"
            "  d. 记录创建得到的 doc_url（b 或 c 步骤的响应中会包含），"
            "供下一步使用\n"
